...
public abstract class BaseArrangeModel(HttpStatusCode code)
{
...
public abstract class BaseArrangeModel<T1>(HttpStatusCode code) : BaseExpectedModel(code)
{
    public required Action<T1?> AssertOperationResult { get; init; }
}

public abstract class BaseArrangeModel<T1, T2>(HttpStatusCode code) : BaseExpectedModel(code)
{
